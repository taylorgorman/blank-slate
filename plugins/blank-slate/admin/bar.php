...
//-----------------------------------------------------------------------------
	...
	public static function show_admin_bar() {
		return apply_filters('bs_show_admin_bar', static::$show_admin_bar);
	}
...
//-----------------------------------------------------------------------------
	...
	private static function remove_admin_bar_nodes($bar) {
		$nodes_to_remove = apply_filters('bs_nodes_to_remove', static::$nodes_to_remove);
	...
	private static function add_admin_bar_nodes($bar) {
		...
		$greetings = apply_filters('bs_admin_bar_greetings', static::$greetings);
		...
		$nodes_to_add[] = array(
			...
			'title' => sprintf(
				...
				apply_filters('bs_admin_bar_title', get_bloginfo('name'))
			),
			'href'  => apply_filters('bs_admin_bar_url', is_admin() ? get_site_url() : get_admin_url()),
		);
		...
		$nodes_to_add = apply_filters('bs_nodes_to_add', $nodes_to_add);
...
//-----------------------------------------------------------------------------
	...
	** @access public
	public static function change_admin_bar_logo() {
		...
		$logo_image = apply_filters('bs_admin_bar_logo', BS_URL.$default_logo);
