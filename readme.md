...
`ID` *(integer) (optional)* - Post type ID. Fallback: `singular_name`
`singular_name` *(string) (required)* - Used to generate labels.
`plural_name` *(string) (optional)* - Used to generate labels. Fallback: singular_name
`labels` *(array) (optional)* - Plugs directly into `register_post_type`'s labels argument. Default is auto-generated by `singular_name` and `plural_name`
`supports` *(array) (optional)* - Plugs directly into `register_post_type`'s supports argument. Some are auto-set by arguments.
`arguments` *(array) (optional)* - Plugs directly into `register_post_type`'s args parameter. A good way to override anything or everything.
